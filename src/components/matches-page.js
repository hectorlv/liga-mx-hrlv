...
import '../components/match-detail-page.js';
...
class MatchesPage extends LitElement {
  ...
  _toggleRowMenu(e) {
    ...
    this.updateComplete.then(() => {
      if (this.openRowMenuId === id) {
        const menu = this.shadowRoot.querySelector(`.row-menu`);
        if (menu) {
        ...
        const btn = this.shadowRoot.querySelector(`md-icon-button[data-id="${id}"]`);
  ...
  _onRowMenuKeydown(e) {
    ...
    const focusable = Array.from(menu.querySelectorAll('button, [role="menuitem"]')).filter(n => !n.hasAttribute('disabled'));
    ...
    if (e.key === 'Tab') {
      ...
      const idAttr = menu.parentElement?.querySelector('md-icon-button')?.getAttribute('data-id');
