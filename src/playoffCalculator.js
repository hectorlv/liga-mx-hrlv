...
export function calculatePlayIn(table, matches) {
    ...
    const playIn3 = {};
    const playIn1Match = matches.find(
        x => x.idMatch === LIGUILLA.playIn1.id,
    );
    const playIn2Match = matches.find(
...
function calculateQuarterFinal(table, matches) {
    ...
    quarter1[`/matches/${LIGUILLA.quarter1.vuelta.id}/estadio`] =
        getEstadio(quarters[LIGUILLA.quarter1.local].equipo);
    const quarter2 = {};
    ...
    quarter2[`/matches/${LIGUILLA.quarter2.vuelta.id}/estadio`] =
        getEstadio(quarters[LIGUILLA.quarter2.local].equipo);
    const quarter3 = {};
    ...
    quarter3[`/matches/${LIGUILLA.quarter3.vuelta.id}/estadio`] =
        getEstadio(quarters[LIGUILLA.quarter3.local].equipo);
    const quarter4 = {};
    ...
    quarter4[`/matches/${LIGUILLA.quarter4.vuelta.id}/estadio`] =
        getEstadio(quarters[LIGUILLA.quarter4.local].equipo);
...
function calculateSemiFinal(table, matches) {
    const quarter1 = {
        local: matches.find(x => x.idMatch === LIGUILLA.quarter1.ida.id)
            .visitante,
        visitante: matches.find(x => x.idMatch === LIGUILLA.quarter1.ida.id)
            .local,
        golLocal:
            matches.find(x => x.idMatch === LIGUILLA.quarter1.ida.id)
                .golVisitante +
            matches.find(x => x.idMatch === LIGUILLA.quarter1.vuelta.id)
                .golLocal,
        golVisitante:
            matches.find(x => x.idMatch === LIGUILLA.quarter1.ida.id)
                .golLocal +
    ...
    const quarter2 = {
        local: matches.find(x => x.idMatch === LIGUILLA.quarter2.ida.id)
            .visitante,
        visitante: matches.find(x => x.idMatch === LIGUILLA.quarter2.ida.id)
            .local,
        golLocal:
            matches.find(x => x.idMatch === LIGUILLA.quarter2.ida.id)
                .golVisitante +
            matches.find(x => x.idMatch === LIGUILLA.quarter2.vuelta.id)
                .golLocal,
        golVisitante:
            matches.find(x => x.idMatch === LIGUILLA.quarter2.ida.id)
                .golLocal +
    ...
    const quarter3 = {
        local: matches.find(x => x.idMatch === LIGUILLA.quarter3.ida.id)
            .visitante,
        visitante: matches.find(x => x.idMatch === LIGUILLA.quarter3.ida.id)
            .local,
        golLocal:
            matches.find(x => x.idMatch === LIGUILLA.quarter3.ida.id)
                .golVisitante +
            matches.find(x => x.idMatch === LIGUILLA.quarter3.vuelta.id)
                .golLocal,
        golVisitante:
            matches.find(x => x.idMatch === LIGUILLA.quarter3.ida.id)
                .golLocal +
    ...
    const quarter4 = {
        local: matches.find(x => x.idMatch === LIGUILLA.quarter4.ida.id)
            .visitante,
        visitante: matches.find(x => x.idMatch === LIGUILLA.quarter4.ida.id)
            .local,
        golLocal:
            matches.find(x => x.idMatch === LIGUILLA.quarter4.ida.id)
                .golVisitante +
            matches.find(x => x.idMatch === LIGUILLA.quarter4.vuelta.id)
                .golLocal,
        golVisitante:
            matches.find(x => x.idMatch === LIGUILLA.quarter4.ida.id)
                .golLocal +
...
function calculateFinal(table, matches) {
    const semis1 = {
        local: matches.find(x => x.idMatch === LIGUILLA.semi1.ida.id)
            .visitante,
        visitante: matches.find(x => x.idMatch === LIGUILLA.semi1.ida.id)
            .local,
        golLocal:
            matches.find(x => x.idMatch === LIGUILLA.semi1.ida.id)
                .golVisitante +
            matches.find(x => x.idMatch === LIGUILLA.semi1.vuelta.id).golLocal,
        golVisitante:
            matches.find(x => x.idMatch === LIGUILLA.semi1.ida.id).golLocal +
            matches.find(x => x.idMatch === LIGUILLA.semi1.vuelta.id)
                .golVisitante,
    };
    const semis2 = {
        local: matches.find(x => x.idMatch === LIGUILLA.semi2.ida.id)
            .visitante,
        visitante: matches.find(x => x.idMatch === LIGUILLA.semi2.ida.id)
            .local,
        golLocal:
            matches.find(x => x.idMatch === LIGUILLA.semi2.ida.id)
                .golVisitante +
            matches.find(x => x.idMatch === LIGUILLA.semi2.vuelta.id).golLocal,
        golVisitante:
            matches.find(x => x.idMatch === LIGUILLA.semi2.ida.id).golLocal +
            matches.find(x => x.idMatch === LIGUILLA.semi2.vuelta.id)
                .golVisitante,
    };

    if (semis1.golLocal >= semis1.golVisitante) {
        table.find(team => team.equipo === semis1.visitante).eliminado =
            true;
    } else {
        table.find(team => team.equipo === semis1.local).eliminado = true;
    }
    if (semis2.golLocal >= semis2.golVisitante) {
        table.find(team => team.equipo === semis2.visitante).eliminado =
            true;
    } else {
